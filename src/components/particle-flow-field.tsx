...
function noise2(xin: number, yin: number): number {
  ...
  if (tt >= 0) { tt *= tt; n += tt * tt * (_GX[gi0]! * x0 + _GY[gi0]! * y0) }
  tt = 0.5 - x1*x1 - y1*y1
  if (tt >= 0) { tt *= tt; n += tt * tt * (_GX[gi1]! * x1 + _GY[gi1]! * y1) }
  tt = 0.5 - x2*x2 - y2*y2
  if (tt >= 0) { tt *= tt; n += tt * tt * (_GX[gi2]! * x2 + _GY[gi2]! * y2) }
  return 70 * n
}
// ────────────────────────────────────────────────────────────────────
...
interface P { x: number; y: number; vx: number; vy: number; rx: number; ry: number }
      ...
      for (let i = 0; i < n; i++) {
        ...
        for (let j = i + 1; j < n; j++) {
          ...
          if (d2 < CONN_DIST_SQ) {
            ...
            const bucket = Math.min(BUCKETS - 1, (alpha / LINE_ALPHA * BUCKETS) | 0)
